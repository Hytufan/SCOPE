...
import warnings
warnings.filterwarnings("ignore", category=UserWarning, message="No positive samples in y_true")
...
def train(
    ...
    eval_modules = (
        ...
        model.backbone,
        model.roi_heads.box,
        model.roi_heads.relation.protected_predictor,
        #model.roi_heads.relation.protected_predictor.scene_recognizor,
        ...
        #model.roi_heads.relation.predictor.rel_context_layer,
        #model.roi_heads.relation.predictor.rel_classifier,
    )
    train_modules = ()
    rel_pn_module_ref = []
    if cfg.PECONFIG.TRAIN_LABEL:
        eval_modules = [
                model.rpn,
                model.backbone,
                model.roi_heads.box,
    ...
    if cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR == "IMPPredictor":
        ...
        if cfg.MODEL.ROI_RELATION_HEAD.BGNN_MODULE.RELATION_CONFIDENCE_AWARE:
            slow_heads = [
                ...
                #"roi_heads.relation.predictor.rel_context_layer.relation_conf_aware_models",
                #"roi_heads.relation.predictor.obj_context_layer.relation_conf_aware_models",
            ]        
    ...
    if cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR == "BGNN_MODULE":
        if (
            ...
            except_weight_decay = [
                #"roi_heads.relation.protected_predictor.ggnn",
                #"roi_heads.relation.predictor.obj_context_layer.learnable_relness_score_gating_recalibration",
    ...
    load_mapping = {
        ...
        #"roi_heads.relation.predictor.rel_context_layer":"roi_heads.relation.predictor.context_layer",
        #"roi_heads.relation.predictor.obj_context_layer":"roi_heads.relation.predictor.context_layer",
    }
    ...
    if cfg.MODEL.PRETRAINED_DETECTOR_CKPT != "":
        ...
        if cfg.PECONFIG.HIDESEEKER:
            scene_state_dict = torch.load('./checkpoints/detection/pretrained_faster_rcnn/resnet50_places365.pt', map_location=device)
            model.roi_heads.relation.protected_predictor.scene_recognizor.load_state_dict(scene_state_dict)
            logger.info("Load from ./checkpoints/detection/pretrained_faster_rcnn/resnet50_places365.pt!")
    else:
        print("No Mapping!")
        checkpointer.load(
            cfg.MODEL.WEIGHT,
    ...
    if (
        ...
        and not cfg.PECONFIG.TRAIN_LABEL
        and not cfg.PROTECTED.TRAINING
    ):
        if distributed:
            m2opt = model.module
        else:
    ...
    for iteration, (images, targets, protect_targets) in enumerate(train_data_loader, start_iter):
        ...
        protect_targets = [protect_target.to(device) for protect_target in protect_targets]
        loss_dict = model(images, targets, protect_targets, logger=logger)
        #continue
        #loss_dict={"loss_refine_obj":loss_dict["loss_refine_obj"]}
    
        losses = sum(loss for loss in loss_dict.values())
        #losses -= loss_dict["loss_refine_obj"]*0.5
        #break
...
def run_val(cfg, model, val_data_loaders, distributed, logger):
    ...
    if cfg.MODEL.RELATION_ON and not cfg.PECONFIG.TRAIN_LABEL and not cfg.PROTECTED.TRAINING:
...
def run_test(cfg, model, distributed, logger):
    ...
    if cfg.MODEL.KEYPOINT_ON:
        iou_types = iou_types + ("keypoints",)
    if cfg.MODEL.RELATION_ON and not cfg.PROTECTED.TRAINING:
        iou_types = iou_types + ("relations",)
    if cfg.MODEL.ATTRIBUTE_ON:
        iou_types = iou_types + ("attributes",)
...
def main():
    ...
    cfg.OUTPUT_DIR = os.path.join(
        ...
        f"{time_str}_{cfg.EXPERIMENT_NAME}"
