...
def show_params_status(model):
    ...
    for p_name, (size, prod, p_req_grad) in st.items():
        if len(size)==0:
            size=['1','1']
            prod=1
        #print(p_name, size,prod, p_req_grad)
        strings.append(
            "{:<80s}: {:<16s}({:8d}) ({})".format(
...
def train(
    ...
    train_modules = ()
    '''eval_modules = [
                model.relationNet.rpn,
                model.relationNet.backbone,
                model.relationNet.roi_heads.box,
    ...
    if cfg.KGEVENT.CLIP:
        eval_modules = [
                    ...
                    model.backbone,
                    model.roi_heads.box,
                    model.roi_heads.scene_recognizor,
                    model.roi_heads.event.event_predictor.ggnn.clip_model,
                    #model.roi_heads.relation.predictor,
                ]
    else:
        eval_modules = [
                    ...
                    #model.roi_heads.event.event_predictor.ggnn.clip_model,
                    #model.roi_heads.relation.predictor,
                    #model.roi_heads.event.event_predictor.ggnn.event_model,
                ]
    ...
    if (
        ...
        if distributed:
            ...
            m2opt = model
        m2opt.relationNet.roi_heads.relation.predictor.start_preclser_relpn_pretrain()
    ...
    for iteration, (images, targets, protect_targets) in enumerate(train_data_loader, start_iter):
        ...
        #print("start:",datetime.datetime.now().strftime("%H:%M:%S.{:03d}".format(datetime.datetime.now().microsecond // 1000)))
        loss_dict = model(images, targets, protect_targets, logger=logger)
        #continue
        #loss_dict={"loss_refine_obj":loss_dict["loss_refine_obj"]}
        #losses = sum(loss for loss in loss_dict.values())
        losses = 0.9 * loss_dict['loss_event'] + 0.1 * loss_dict['loss_obj']
        #print(loss_dict)
        ...
        #print("backward:",datetime.datetime.now().strftime("%H:%M:%S.{:03d}".format(datetime.datetime.now().microsecond // 1000)))
        ...
        if pre_clser_pretrain_on:
            if iteration == STOP_ITER:
                ...
                m2opt.relationNet.roi_heads.relation.predictor.end_preclser_relpn_pretrain()
                pre_clser_pretrain_on = False
