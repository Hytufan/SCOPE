...
def merge_boxlists(boxlist1, boxlist2):
    bbox1 = boxlist1.bbox
    bbox2 = boxlist2.bbox
    extra_fields1 = boxlist1.extra_fields
    extra_fields2 = boxlist2.extra_fields

    merged_bbox = torch.cat((bbox1, bbox2), dim=0)

    merged_boxlist = BoxList(merged_bbox, boxlist1.size, mode=boxlist1.mode)
    merged_extra_fields = {**extra_fields1, **extra_fields2}

    for field, data in merged_extra_fields.items():
        if field in merged_boxlist.extra_fields:
            merged_boxlist.extra_fields[field] = torch.cat((merged_boxlist.extra_fields[field], data), dim=0)
        else:
            merged_boxlist.extra_fields[field] = data
