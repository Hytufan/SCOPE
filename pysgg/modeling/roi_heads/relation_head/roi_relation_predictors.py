...
class BGNNPredictor(nn.Module):
    def __init__(self, config, in_channels):
        ...
        self.series_parallel=config.PECONFIG.SERIES_PARALLEL
        self.use_avg_feature=config.PECONFIG.USE_AVG_FEATURE
        self.new_order=config.PECONFIG.NEW_ORDER
        self.hideseeker=cfg.PECONFIG.HIDESEEKER
        self.node_num=cfg.PROTECTED.GGNN_N_NODES - 2
        self.feature_mode=cfg.PECONFIG.FEATURE_INPUT

        if self.hideseeker:
            if self.feature_mode=='cat':
                self.roi_fc = nn.Sequential(
                    make_fc(cfg.MODEL.ROI_BOX_HEAD.MLP_HEAD_DIM, cfg.MODEL.ROI_BOX_HEAD.MLP_HEAD_DIM // 2),
                    nn.ReLU(True),
                )
                ...
                self.cat_fc=nn.Sequential(
                    make_fc(cfg.PROTECTED.GGNN_D_STATE, cfg.PROTECTED.GGNN_D_STATE),
                    nn.ReLU(True),
                )
            elif self.feature_mode=='replace':
                self.node_fc=nn.Sequential(
                    make_fc(cfg.PROTECTED.GGNN_D_STATE, cfg.PROTECTED.GGNN_D_STATE),
                    nn.ReLU(True),
                )
        ...
        if self.new_order:
            self.pro_rel_classifier = build_classifier(self.hidden_dim, self.num_rel_cls)
    ...
    def start_preclser_relpn_pretrain(self):
        if not self.split_context_model4inst_rel:
            self.context_layer.set_pretrain_pre_clser_mode()
        else:
            self.rel_context_layer.set_pretrain_pre_clser_mode()
            self.obj_context_layer.set_pretrain_pre_clser_mode()

    def end_preclser_relpn_pretrain(self):
        if not self.split_context_model4inst_rel:
            self.context_layer.set_pretrain_pre_clser_mode(False)
        else:
            self.rel_context_layer.set_pretrain_pre_clser_mode(False)
            self.obj_context_layer.set_pretrain_pre_clser_mode(False)
    ...
    def forward(
        ...
        if self.hideseeker:
            nodes_other=None
            for proposal, nodes_opt in zip(inst_proposals,nodes_output):
                node_num=len(proposal)
                if nodes_other==None:
                    nodes_other=nodes_opt[2:node_num+2,:]
                else:
                    nodes_other=torch.cat((nodes_other,nodes_opt[2:node_num+2,:]))
            #print(roi_features.size(),nodes_other.size())
            if self.feature_mode=='cat':
                roi_features=self.roi_fc(roi_features)
                nodes_other=self.node_fc(nodes_other)
                roi_features=torch.cat((roi_features, nodes_other), dim=1)
                roi_features=self.cat_fc(roi_features)
            elif self.feature_mode=='replace':
                nodes_other=self.node_fc(nodes_other)
                roi_features=nodes_other
        ...
        if self.split_context_model4inst_rel:
            ...
            #print(f'obj:{obj_feats.size()}\nrel:{rel_feats.size()}\nroi:{roi_features.size()}\n')
        else:
            obj_feats, rel_feats, pre_cls_logits, relatedness = self.context_layer(
                roi_features, union_features, inst_proposals, rel_pair_idxs, rel_binarys, logger
            )
        ...
        if self.split_context_model4inst_rel:
            ...
            if self.series_parallel=='series':
                obj_refine_logits_ori = [prop.get_field("predict_logits") for prop in inst_proposals]
                obj_fuse_logits=[]
                for ori_logits,ref_logits,proposal in zip(obj_refine_logits_ori,refined_obj_logits,inst_proposals):
                ...
                for i in range(len(inst_proposals)):
                    inst_proposals[i].add_field("predict_logits",obj_fuse_logits[i])
                ...
                if self.use_avg_feature:
                    ...
                    refined_obj_pred_labels=refined_obj_pred_labels.detach().cpu().numpy().tolist()
                    is_protect=cat(
                        [proposal.get_field("is_protect") for proposal in inst_proposals], dim=0
                    )
                    is_protect=is_protect.detach().cpu().numpy().tolist()

                    avg_file='/datapool/workspace/huangyunyi/code/SceneGraph/PySGG/checkpoints/avg_feature_4.pt'
                    device = roi_features.device
                    if os.path.exists(avg_file):
                        feature_dict = torch.load(avg_file,map_location=device)
                    for i in range(roi_features.size()[0]):
                        if is_protect[i]==1:
                            label=refined_obj_pred_labels[i]
                            roi_features[i]=feature_dict[label]["feat"].to(device)
                
            _, rel_feats, pre_cls_logits, relatedness = self.rel_context_layer(
                roi_features, union_features, inst_proposals, rel_pair_idxs, rel_binarys, logger
            )
        ...
        rel_cls_logits = self.rel_classifier(rel_feats)
        if self.new_order:
            pro_rel_cls_logits = self.pro_rel_classifier(rel_feats)
        ...
        rel_cls_logits = rel_cls_logits.split(num_rels, dim=0)
        if self.new_order:
            pro_rel_cls_logits = pro_rel_cls_logits.split(num_rels, dim=0)
            rel_fuse_logits=[]
            device=rel_cls_logits[0].device
            for rel_cls_logit, pro_rel_cls_logit, proposal, rel_pair_idx in zip(rel_cls_logits,pro_rel_cls_logits,inst_proposals,rel_pair_idxs):
                is_protect=proposal.get_field("is_protect").detach().cpu().numpy().tolist()
                protect_index=[i for i in range(len(is_protect)) if is_protect[i] == 1]
                pair_is_protect=[]
                for rel_pair in rel_pair_idx:
                    rel_pair=rel_pair.detach().cpu().numpy().tolist()
                    if rel_pair[0] in protect_index or rel_pair[1] in protect_index:
                        pair_is_protect.append(1)
                    else:
                        pair_is_protect.append(0)
                rel_fuse_logit=torch.where(torch.tensor(pair_is_protect).to(device).unsqueeze(1) == 1, pro_rel_cls_logit, rel_cls_logit)
                rel_fuse_logits.append(rel_fuse_logit)
            rel_cls_logits=tuple(rel_fuse_logits)
        ...
        bgnn_feats=dict(rel_feats=rel_feats,obj_feats=obj_feats)
        
        return obj_pred_logits, rel_cls_logits, add_losses, bgnn_feats
