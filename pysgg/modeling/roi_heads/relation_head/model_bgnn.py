...
class BGNNContext(nn.Module):
    def __init__(
        ...
        if pred_type=='obj' and cfg.PECONFIG.TRAIN_LABEL:
            self.rel_aware_on=(False)
    ...
    def forward(
        ...
        if len(relatedness_each_iters)==1 and relatedness_each_iters[0] is None:
            relatedness_each_iters=[]
        if (
            len(relatedness_each_iters) > 0 and not self.training and not self.cfg.EVENT.TRAINING
