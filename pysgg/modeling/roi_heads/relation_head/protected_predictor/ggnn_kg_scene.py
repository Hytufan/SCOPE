...
class GGNN(nn.Module):
    def __init__(self, cfg, emb_path, graph_path, time_step_num=3, hidden_dim=512, output_dim=512, 
                 use_embedding=True, use_knowledge=True, refine_obj_cls=True, top_k_to_keep=5, normalize_messages=True):
        ...
        if use_knowledge:
            kg_path='/workspace/huangyunyi/data/WIDER_v0.1/KG/vise_wider_npy/'
        ...
        self.fc_init_img_sce = nn.Linear(2048, hidden_dim)
        ...
        self.fc_mp_receive_ont_eve = MLP([(self.num_edge_types_obj2eve + self.num_edge_types_eve2eve + self.num_edge_types_sce2eve + 1) * hidden_dim // 4, 
                                           (self.num_edge_types_obj2eve + self.num_edge_types_eve2eve + self.num_edge_types_sce2eve + 1) * hidden_dim // 4, 
                                           hidden_dim], act_fn='ReLU', last_act=True)
        self.fc_mp_receive_ont_sce = MLP([(self.num_edge_types_eve2sce + 1) * hidden_dim // 4, 
                                           (self.num_edge_types_eve2sce + 1) * hidden_dim // 4, 
                                           hidden_dim], act_fn='ReLU', last_act=True)
        ...
        self.fc_mp_receive_img_sce = MLP([2 * hidden_dim // 4, 3 * hidden_dim // 4, hidden_dim], act_fn='ReLU', last_act=True)
    ...
    def forward(self, proposal, roi_feature, scene_info, image=None):
        
        device=roi_feature.device
        ...
        edges_img_obj2obj = wrap(np.ones((num_img_obj, num_img_obj)))
