...
class PairwiseFeatureExtractor(nn.Module):
    ...
    def __init__(self, config, in_channels):
        ...
        self.use_pro_obj_vis_feat=config.PECONFIG.USE_PROTECTED_OBJ_VISUAL_FEATURE
        if self.use_pro_obj_vis_feat:
        ...
        '''self.use_pro_obj_vis_feat=config.PECONFIG.USE_PROTECTED_OBJ_VISUAL_FEATURE
        if self.use_pro_obj_vis_feat == False:
            self.pro_obj_hidden_linear=make_fc(self.embed_dim + self.geometry_feat_dim, self.hidden_dim)'''
    ...
    def forward(self, inst_roi_feats, union_features, inst_proposals, rel_pair_idxs, ):
        ...
        """
        '''print(inst_proposals)
        print(inst_proposals[0].fields())
        print(inst_roi_feats.shape)
        ...
        if self.use_pro_obj_vis_feat:
            ...
            if self.word_embed_feats_on:
                obj_pre_rep = cat((obj_embed_by_pred_dist, pos_embed), -1)
            else:
                obj_pre_rep = cat((pos_embed), -1)
        #print(obj_pre_rep.size())
        ...
        if self.use_pro_obj_vis_feat:
            ...
            if self.word_embed_feats_on:
                augment_obj_feat = cat((obj_embed_by_pred_labels, augment_obj_feat), -1)
            else:
                augment_obj_feat = cat((augment_obj_feat), -1)
