...
#from .protected_predictor.ggnn_event_protected_explain import GGNNEventReason
from .protected_predictor.ggnn_event_protected import GGNNEventReason
import datetime
...
class ROIPAEventHead(torch.nn.Module):
    """
    Generic Relation Head class.
    """

    def __init__(self, cfg, in_channels):
        super(ROIPAEventHead, self).__init__()
        self.cfg = cfg.clone()
        ...
        if cfg.PROTECTED.FEATURE:
            self.protected_feature_extractor = make_roi_box_feature_extractor(cfg, in_channels, half_out=True)
            feat_dim_protected = self.box_feature_extractor.out_channels
            if isinstance(self.box_feature_extractor, ResNet50Conv5ROIFeatureExtractor):
                feat_dim_protected = self.box_feature_extractor.flatten_out_channels
        ...
        self.event_predictor=GGNNEventReason(cfg)
    ...
    def forward(self, features, proposals, targets=None, protect_targets=None, scene_info_list=None, logger=None):
        ...
        if self.cfg.PROTECTED.FEATURE:
            roi_features_protected = self.protected_feature_extractor(features, protect_targets)
            if isinstance(self.protected_feature_extractor, ResNet50Conv5ROIFeatureExtractor):
                roi_features_protected = self.protected_feature_extractor.flatten_roi_features(roi_features_protected)

        #print("roi_feature:",datetime.datetime.now().strftime("%H:%M:%S.{:03d}".format(datetime.datetime.now().microsecond // 1000)))
        ...
        if 'is_protect' not in proposals[0].fields():
            proposals=assign_isprotect_to_proposals(proposals,protect_targets)

        proposals, protect_loss, nodes_output=self.event_predictor(proposals,protect_targets,roi_features,roi_features_protected,scene_info_list)
        if self.training:
            #output_losses = dict(loss_event=protect_loss[0])
            if len(protect_loss)>1:
...
def build_pa_roi_event_head(cfg, in_channels):
    """
    ...
    """
    return ROIPAEventHead(cfg, in_channels)
