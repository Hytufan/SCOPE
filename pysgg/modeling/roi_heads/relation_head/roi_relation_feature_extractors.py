...
class RelationFeatureExtractor(nn.Module):
    ...
    def __init__(self, cfg, in_channels):
        ...
        self.use_protect_union_mask=cfg.PECONFIG.USE_PROTECT_UNION_MASK
        self.union_background_rate=cfg.PECONFIG.UNION_BACKGROUND_RATE
        self.separate_union_mask=cfg.PECONFIG.SEPARATE_UNION_MASK
        ...
        if self.geometry_feature:
            ...
            if self.separate_union_mask:
                input_size = self.feature_extractor.resize_channels
                out_dim = self.feature_extractor.out_channels
                self.union_mask_fc = nn.Sequential(*[make_fc(input_size, out_dim // 4), nn.ReLU(inplace=True),
                                                  make_fc(
                                                      out_dim // 4, out_dim), nn.ReLU(inplace=True),
                                                  ])
                self.fusion_union_mask_fc=nn.Sequential(*[make_fc(out_dim*2, out_dim // 4), nn.ReLU(inplace=True),
                                                          make_fc(out_dim // 4, out_dim), nn.ReLU(inplace=True),])
        if self.use_protect_union_mask:
            self.mask_rect_conv =nn.Sequential(*[
                nn.Conv2d(1, in_channels // 2, kernel_size=7,
                          stride=2, padding=3, bias=True),
                nn.ReLU(inplace=True),
                nn.BatchNorm2d(in_channels // 2, momentum=0.01),
                nn.MaxPool2d(kernel_size=3, stride=2, padding=1),
    ...
    def forward(self, x, proposals, rel_pair_idxs=None):
        ...
        for proposal, rel_pair_idx in zip(proposals, rel_pair_idxs):
            ...
            if self.use_protect_union_mask:
                pro_bbox=get_pro_bbox(proposal)
                pro_bboxes=np.tile(pro_bbox, (num_rel, 1))
                pro_bboxes=torch.from_numpy(pro_bboxes).to(device)
                pro_proposal=BoxList(pro_bboxes, proposal.size, mode='xyxy')

                is_protect=proposal.get_field("is_protect").detach().cpu().numpy()
                indices = [index for index, value in enumerate(is_protect) if value == 1]
                pair1=rel_pair_idx[:, 0].detach().cpu().numpy()
                pair2=rel_pair_idx[:, 1].detach().cpu().numpy()
                union_has_protect=[1 if (value1 in indices) or (value2 in indices) else 0 for value1, value2 in zip(pair1, pair2)]
                union_has_protect=torch.tensor(union_has_protect, dtype=torch.float32).to(device)

                union_proposal=union_proposal.resize(
                    (self.rect_size, self.rect_size))
                pro_proposal=pro_proposal.resize(
                    (self.rect_size, self.rect_size))
                union_rect = ((dummy_x_range >= union_proposal.bbox[:, 0].floor().view(-1, 1, 1).long())
                            & (dummy_x_range <= union_proposal.bbox[:, 2].ceil().view(-1, 1, 1).long())
                            & (dummy_y_range >= union_proposal.bbox[:, 1].floor().view(-1, 1, 1).long())
                            & (dummy_y_range <= union_proposal.bbox[:, 3].ceil().view(-1, 1, 1).long())).bool()
                protect_rect = ((dummy_x_range >= pro_proposal.bbox[:, 0].floor().view(-1, 1, 1).long())
                            & (dummy_x_range <= pro_proposal.bbox[:, 2].ceil().view(-1, 1, 1).long())
                            & (dummy_y_range >= pro_proposal.bbox[:, 1].floor().view(-1, 1, 1).long())
                            & (dummy_y_range <= pro_proposal.bbox[:, 3].ceil().view(-1, 1, 1).long())).bool()
                mask_rect = (~protect_rect) & union_rect
                mask_rect = mask_rect.float()

                background_rect=(~union_rect).float() * self.union_background_rate
                background_rect=background_rect * union_has_protect.view(-1, 1, 1)

                mask_rect=mask_rect+background_rect

                # (num_rel, 1, rect_size, rect_size)
                #rect_input = torch.stack((head_rect, tail_rect), dim=1)
                mask_rect_inputs.append(mask_rect.unsqueeze(1))
        ...
        if self.geometry_feature:
            ...
            if self.use_protect_union_mask:
                mask_rect_inputs=torch.cat(mask_rect_inputs, dim=0)
                mask_rect_features = self.mask_rect_conv(mask_rect_inputs)

                '''attention_scores = torch.matmul(union_vis_features, mask_rect_features.transpose(2, 3))
                attention_scores = F.softmax(attention_scores, dim=-1)
                weighted_features = torch.matmul(attention_scores, union_vis_features)
            ...
            if self.separate_spatial:
                ...
                if self.use_protect_union_mask and not self.separate_union_mask:
                    union_features = union_features + mask_rect_features
                union_features = self.feature_extractor.forward_without_pool(union_features)
                if self.separate_union_mask:
                    mask_features=self.union_mask_fc(mask_rect_features.view(mask_rect_features.size(0), -1))
                    union_features=torch.cat((union_features, mask_features), dim=-1)
                    union_features=self.fusion_union_mask_fc(union_features)
...
def get_pro_bbox(proposal):
    is_protect=proposal.get_field("is_protect").detach().cpu().numpy()
    protect_bbox=proposal.bbox.detach().cpu().numpy()[is_protect==1]
    bbox=np.mean(protect_bbox, axis=0)
    return bbox
