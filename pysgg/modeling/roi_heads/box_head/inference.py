...
class PostProcessor(nn.Module):
    ...
    def add_important_fields(self, i, boxes, orig_inds, boxlist, boxes_per_cls, relation_mode=False):
        if relation_mode and self.training:
            ...
            gt_isprotect=boxes[i].get_field('is_protect')[orig_inds]

            boxlist.add_field('labels', gt_labels)
            boxlist.add_field('attributes', gt_attributes)
            boxlist.add_field('is_protect', gt_isprotect)
    ...
    def filter_results(self, boxlist, num_classes, obj_logit=None):
        ...
        if not self.background_on:
            ...
            start_ind=0
        for j in range(start_ind, num_classes):
        ...
        if self.nms_filter_duplicates or self.save_proposals:
            ...
            for j in range(start_ind, num_classes):
                inds_all[:, j] = 0
                orig_idx = orig_inds[j - start_ind]
        ...
        if not self.background_on:
            ...
            # 优先选择 score > 0.5 且 class != 0 的对象
            scores = result.get_field("pred_scores")
            labels = result.get_field("pred_labels")

            # 选择 score > 0.5 且 class != 0 的对象
            high_score_mask = (scores > 0.5) & (labels != 0)
            high_score_indices = torch.nonzero(high_score_mask).squeeze(1)
            high_score_result = result[high_score_indices]
            high_score_orig_inds = orig_inds[high_score_indices]

            # 如果这些对象的数量已经足够，则直接返回
            if len(high_score_result) >= self.detections_per_img:
                result = high_score_result[:self.detections_per_img]
                orig_inds = high_score_orig_inds[:self.detections_per_img]
            else:
                # 选择 class == 0 的对象
                low_score_mask = (scores > 0.5) & (labels == 0)
                low_score_indices = torch.nonzero(low_score_mask).squeeze(1)
                low_score_scores = scores[low_score_indices]

                # 选择 score 前 k 大的对象
                k = self.detections_per_img - len(high_score_result)
                if len(low_score_scores) > k:
                    _, topk_indices = torch.topk(low_score_scores, k)
                    low_score_indices = low_score_indices[topk_indices]

                # 合并高分对象和低分对象
                combined_indices = torch.cat([high_score_indices, low_score_indices])
                result = result[combined_indices]
                orig_inds = orig_inds[combined_indices]

            # 确保最终结果的数量不超过 self.detections_per_img
            if len(result) > self.detections_per_img:
                result = result[:self.detections_per_img]
                orig_inds = orig_inds[:self.detections_per_img]
...
def make_roi_box_post_processor(cfg):
    ...
    background_on = cfg.KGEVENT.BACKGROUND_ON
