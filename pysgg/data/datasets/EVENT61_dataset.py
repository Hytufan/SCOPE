...
class EVENT61Dataset(torch.utils.data.Dataset):
    def __init__(self, split, img_dir, filename_file_path, dict_file, roidb_file, transforms=None,
        ...
        for i in range(len(self.filenames)):
            if self.test_mode:
                ...
                image = Image.open(os.path.join(self.image_dir+self.filenames[i]).replace(' ',''))
    ...
    def __getitem__(self, idx):
        if not self.test_mode:
            img = Image.open(os.path.join(self.image_dir+self.filenames[idx]).replace(' ','')).convert("RGB")
        else:
    ...
    def get_protect_target(self,index,img_size):
        ...
        pro_labels= self.protect_info[self.filenames[index]]["labels"]
        pro_bboxes = self.protect_info[self.filenames[index]]["bboxes"]
...
def apply_gaussian_blur(image, bounding_box):
    ...
    # Apply Gaussian blur
    blurred_img = cropped_img.filter(ImageFilter.GaussianBlur(radius=5))
...
def white_box(img,protect_boxlist):
    img = np.array(img)
    x1, y1, x2, y2 = [int(num) for num in protect_boxlist.bbox[0].tolist()]
    img[y1:y2, x1:x2] = [255, 255, 255]
    img = Image.fromarray(img)
...
def apply_mosaic(image, bounding_box, mosaic_size=20):
    # 获取图像的宽度和高度
